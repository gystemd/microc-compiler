...
let string_var_initialization loc vars array_length id string =
  ...
  with DuplicateEntry id ->
    raise
    @@ Semantic_error
         (loc, "Variable " ^ id ^ " already defined in current scope")
  ...
  | Call (id, params) -> (
      ...
      | Some (_, f) -> (
          ...
          | l1, l2 when l1 < l2 ->
              raise
              @@ Semantic_error
                   (e.loc, "Missing one or more arguments in function call")
          | l1, l2 when l1 > l2 ->
              raise
              @@ Semantic_error (e.loc, "Too many arguments in function call")
          | _ ->
              List.iter2
                (fun ft pt ->
              ...
              f.typ)
      | None -> raise @@ Semantic_error (e.loc, "Function " ^ id ^ "not defined")
      )
...
and access_type scope a =
  ...
  | AccVar v -> (
      ...
      | None ->
          raise
          @@ Semantic_error
               (a.loc, "Variable " ^ v ^ " not defined in current scope"))
  | AccDeref e -> (
...
let check_parameter scope loc (t, i) =
  ...
  | _ -> (
      ...
      with DuplicateEntry i ->
        raise
        @@ Semantic_error
             (loc, "Parameter " ^ i ^ " already defined in current scope"))
...
let add_function scope node =
  ...
  | Fundecl f ->
      let new_scope =
        try Symbol_table.add_entry f.fname (node.loc, f) scope.fun_symbols
        with DuplicateEntry d ->
          raise
