...
let handle_semantic_error source code_pos msg =
  ...
  then (
    ...
    let middle =
      String.make (code_pos.Location.end_column - code_pos.Location.start_column + 1) '^'
    in
    Printf.eprintf
      "\n*** Error at line %d.\n%s\n%s%s\n*** %s\n\n"
...
let () =
  try
    ...
    in
    let usage = Printf.sprintf "Usage:\t%s [options] <source_file>\n" Sys.argv.(0) in
    Arg.parse spec_list (fun file -> filename := file) usage;
    if String.equal !filename ""
    then Arg.usage spec_list usage
    else (
